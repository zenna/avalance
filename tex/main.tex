...
Much of hypothesis construction draws first from recognition; we infer constraints on valid proposals by first noticing regularities and structure in data.
In the discovery of equalities, we suggest that there are certain properties that we evaluate, which in when combination can increase or decrease the likelihood of any assertion about the data.
Concretely, given data drawn from the functional relationship $y=sinx+x^2$.
Evaluating attributes may be automatic or deliberative, procedural or parallel, and 
the responsibility of which is distributed among lower level sensory areas.
...
Then task is given a set of attributes $f:D \rightarrow prop$, to evaluate $P(model \vert A)$
...
For these purposes, and we suspect
...
Unlike a context free grammar or stochastic context free grammar, a modification grammar neither recognises strings nor generates new strings; it extends existing strings.
