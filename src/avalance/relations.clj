...
(defn new-find-expr
  ...
  (let [max-n-plots 5
    ...
    ; loop over (samples of) subexpression sets
    (loop [equations [] n-plots-left max-n-plots seen-subexprs []]
      (let [n-subexprs-attempt (inc (rand-int 2))
            expr-constraints? (make-expr-constraints mandatory-exprs)
            subexprs-subexprs-data (gen-subexprs data n-subexprs-attempt expr-constraints? mandatory-exprs seen-subexprs)
            {subexprs :subexprs subexprs-data :subexprs-data} subexprs-subexprs-data
            n-subexprs (count (keys subexprs-data))
            models (filter #(= n-subexprs (count (:vars %))) all-models)
            equations (suggest-ext subexprs-data models attrs model-attr-vals)]
        equations))))